...
if __name__ =="__main__":
    ...
    parser.add_argument("-m", "--model_filepath", nargs="+",type=str,\
            help="model to load", default=["./models/exp105724/model_epann2_clamp100_sd_1337_gen99.h5"])
    ...
    for model_filepath in model_filepaths:
        ...
        fitnesses = []
        trial = 0
        for gravity_x in np.linspace(- args.max_gravity_x, args.max_gravity_x * (1+1/steps)\
                , args.steps):
            yy = 0
            for gravity_y in np.linspace(-args.max_gravity_y, args.max_gravity_y*(1+1/args.steps),\
                    args.steps):
                ...
                #print(fitness)
                plt.plot(fitness)
                print_stats_fit(trial, fitness, total_steps, gravity_x, g_y + gravity_y, clamp=None)
